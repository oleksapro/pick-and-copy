...
`}},B.tableSection={filter:[`thead`,`tbody`,`tfoot`],replacement:function(e){return e}};function V(e){var t=e.parentNode;return t.nodeName===`THEAD`||t.firstChild===e&&(t.nodeName===`TABLE`||ge(t))&&he.call(e.childNodes,function(e){return e.nodeName===`TH`})}function ge(e){var t=e.previousSibling;return e.nodeName===`TBODY`&&(!t||t.nodeName===`THEAD`&&/^\s*$/i.test(t.textContent))}function H(e,t){var n=z.call(t.parentNode.childNodes,t),r=` `;return n===0&&(r=`| `),r+e+` |`}function _e(e){for(var t in e.keep(function(e){return e.nodeName===`TABLE`&&!V(e.rows[0])}),B)e.addRule(t,B[t])}function ve(e){e.addRule(`taskListItems`,{filter:function(e){return e.type===`checkbox`&&e.parentNode.nodeName===`LI`},replacement:function(e,t){return(t.checked?`[x]`:`[ ]`)+` `}})}function ye(e){e.use([L,R,_e,ve])}var U=new A({headingStyle:`atx`,codeBlockStyle:`fenced`});U.use(ye),U.addRule(`absoluteImages`,{filter:`img`,replacement:(e,t)=>{let n=t.alt||``,r=t.getAttribute(`src`)||``,i=t.title||``,a=i?` "${i}"`:``;if(!r)return``;let o;try{o=new URL(r,document.baseURI).href}catch{o=r}return`![${n}](${o}${a})`}});var W=!1,G=null,K=null,q=null,J=null;function be(e){let t=e.tagName.toLowerCase();return e.id?t+=`#${e.id}`:e.classList.length&&(t+=`.${[...e.classList].slice(0,2).join(`.`)}`),t}function xe(){K=document.createElement(`div`),K.style.cssText=`all: initial; position: fixed; inset: 0; z-index: 2147483647; pointer-events: none;`;let e=K.attachShadow({mode:`open`});q=document.createElement(`div`),q.style.cssText=`
  ...
  `,J=document.createElement(`div`),J.style.cssText=`
    ...
    white-space: nowrap;
    display: none;
  `,e.appendChild(q),e.appendChild(J),document.documentElement.appendChild(K)}function Se(){K?.remove(),K=null,q=null,J=null}function Ce(e){let t=e.getBoundingClientRect();q.style.display=`block`,q.style.left=`${t.left}px`,q.style.top=`${t.top}px`,q.style.width=`${t.width}px`,q.style.height=`${t.height}px`,J.textContent=be(e),J.style.display=`block`;let n=t.top>20?t.top-20:t.bottom+2;J.style.left=`${Math.max(t.left,0)}px`,J.style.top=`${n}px`}function Y(e){let t=document.elementFromPoint(e.clientX,e.clientY);!t||t===G||(G=t,Ce(t))}function X(e){e.preventDefault(),e.stopPropagation(),e.stopImmediatePropagation();let t=G;Q(),t&&Te(t)}function Z(e){e.key===`Escape`&&(e.preventDefault(),Q())}function we(){W||(W=!0,xe(),document.documentElement.style.cursor=`crosshair`,document.addEventListener(`mousemove`,Y,!0),document.addEventListener(`click`,X,!0),document.addEventListener(`keydown`,Z,!0))}function Q(){W&&(W=!1,G=null,document.documentElement.style.cursor=``,document.removeEventListener(`mousemove`,Y,!0),document.removeEventListener(`click`,X,!0),document.removeEventListener(`keydown`,Z,!0),Se(),n.default.runtime.sendMessage({type:`inspecting-stopped`}).catch(()=>{}))}async function Te(e){let t=U.turndown(e.outerHTML).trim();try{await navigator.clipboard.writeText(t),$(`Copied as Markdown`)}catch(e){if(window.top===window){console.error(`Pick & Copy: failed to copy markdown`,e),$(`Copy failed — see console`,!0);return}try{let e=await n.default.runtime.sendMessage({type:`relay-clipboard-write`,text:t});if(!e?.success)throw Error(e?.error||`unknown error`);$(`Copied as Markdown`)}catch(e){console.error(`Pick & Copy: failed to copy markdown`,e),$(`Copy failed — see console`,!0)}}}function $(e,t=!1){let n=document.createElement(`div`);n.style.cssText=`all: initial; position: fixed; inset: 0; z-index: 2147483647; pointer-events: none;`;let r=n.attachShadow({mode:`open`}),i=document.createElement(`div`);i.textContent=e,i.style.cssText=`
    position: fixed;
    left: 50%;
    bottom: 32px;
  ...
  `,r.appendChild(i),document.documentElement.appendChild(n),requestAnimationFrame(()=>{i.style.opacity=`1`}),setTimeout(()=>{i.style.opacity=`0`,setTimeout(()=>n.remove(),200)},1800)}n.default.runtime.onMessage.addListener(e=>{if(e?.type===`start-inspect`)return we(),Promise.resolve({inspecting:W});if(e?.type===`write-clipboard`)return navigator.clipboard.writeText(e.text).then(()=>({success:!0}),e=>({success:!1,error:String(e)}))});